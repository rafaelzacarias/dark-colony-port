...
import { consumeTransportFifo, transportHostState } from "./transport-host";
...
export function stepBrowserType37(world: CampaignWorld, frame?: BrowserType37Frame): CampaignWorld {
  ...
  const initialHost = transportHostState(staged);
  ...
  for (const marker of world.scenarioMarkers) {
    ...
    const host = transportHostState(staged);
    ...
    if (fifo.types.length) {
      ...
      actor.status = 0;
      host.registry[marker.rawSlot] = null;
      const bytes = new Uint8Array(staged.entityBytes!);
      bytes[marker.rawSlot * 220 + 0x2c] = 0;
      staged = { ...staged, entityBytes: bytes, transportState: host };
